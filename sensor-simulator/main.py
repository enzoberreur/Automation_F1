...
class F1CarSimulator:
    ...
    def __init__(self, config: Dict):
        ...
        self.http_headers = config.get("http_headers") or {}
        self.delay = max(0.0, 1.0 / self.target_throughput)

    async def _ensure_publisher(self):
        if self.publisher is None:
            self.publisher = HTTPPublisher(endpoint_url=self.http_endpoint, headers=self.http_headers)
...
def _build_http_headers() -> Dict[str, str]:
    """Construit les en-têtes HTTP pour authentifier le stream-processor."""

    api_key = os.getenv("STREAM_PROCESSOR_API_KEY")
    if not api_key:
        return {}

    header_name = os.getenv("STREAM_PROCESSOR_API_KEY_HEADER", "X-Api-Key")
